<nav id="sidebarMenu" class="d-flex col-md-3 col-lg-2 px-3 d-md-block bg-light sidebar collapse">
    <div class="position-sticky pt-3 sidebar-sticky">

        {{-- <h6
            class="sidebar-heading d-flex justify-content-between align-items-center px-3  mb-1 text-muted text-uppercase">
            <span>Add Table</span>
            <a class="link-secondary" href="/admin/all" aria-label="Add a new report">
                <span data-feather="plus-circle" class="align-text-bottom"></span>
            </a>
        </h6> --}}
        
        <ul class="nav flex-column">
            <h6>User</h6>
            <li class="nav-item">
                <a class="nav-link" href="/customer">
                    <span data-feather="info" class="align-text-bottom"></span>
                    customer
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="/about">
                    <span data-feather="info" class="align-text-bottom"></span>
                    developer
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="/about">
                    <span data-feather="info" class="align-text-bottom"></span>
                    agent
                </a>
            </li>
        </ul>

        <ul class="nav flex-column pt-4">
            <h6>Table</h6>
            <li class="nav-item">
                <a class="nav-link" href="/admin/unit/data">
                    <span data-feather="meh" class="align-text-bottom"></span>
                    Unit
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="/admin/dokter/all">
                    <span data-feather="smile" class="align-text-bottom"></span>
                    Property
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="/about">
                    <span data-feather="info" class="align-text-bottom"></span>
                    specification
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="/about">
                    <span data-feather="info" class="align-text-bottom"></span>
                    type
                </a>
            </li>

        </ul>
        {{-- <div class="copyright d-flex align-items-end">
            <p class="text-center mt-lg-5 mx-3 badge text-dark text-wrap">Copyright &copy; 2022 Muhammad Surya Ibrahim. All
                rights reserved.</p>
        </div> --}}


    </div>
</nav>
